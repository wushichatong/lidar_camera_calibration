...
//    cv::Mat img(img_w, img_w, CV_8UC1, cv::Scalar::all(0));
    cv::Mat img(img_w, img_h, CV_8UC3, cv::Scalar(0,0,0));

    for (auto pt: points) {
        int col = (int)(pt.x() / z * focal + img_w/2);
        int row = (int)(- pt.y() / z * focal + img_h/2);  // -Y/Z 加了一个负号, 是为了抵消针孔投影时的倒影效果

        if(col > img_w-1 || col< 0 || row > img_h-1 || row < 0)
            continue;
...
//        std::cout << points.at(id).transpose() <<" "<<points.at(id+1).transpose() <<std::endl;
    ...
    bool newSeg = true;
    LineSeg seg;
    // ROS_INFO("new round %d", id_right);
    for (int i = id_left; i < id_right && nextPt < id_right; i += skip) {
        ...
        // ROS_INFO("dfd %d %d", currentPt, nextPt);

        double d1 = points.at(currentPt).head(2).norm();
        double d2 = points.at(nextPt).head(2).norm();
        // ROS_INFO("index : %d d1 %f d2 %f", i, d1,d2);
        double range_max = 3;
        if(d1 < range_max && d2 < range_max || (fabs(d2 - 1414.213)<0.01 || fabs(d1 - 1414.213)<0.01 ) && nextPt < id_right)    // 有效数据,  激光小于 3 m
        {
            ...
            } else
            {
                ...
                // ROS_INFO("new seg %d  %d %f %d", seg.id_start, seg.id_end, dist.head(2).norm(),seg.id_end-seg.id_start);
                if(dist.head(2).norm() > 0.03
                   && points.at(seg.id_start).head(2).norm() < 1.0
                   && points.at(seg.id_end).head(2).norm() < 1.0
                   && seg.id_end-seg.id_start > 20   )  // 至少长于 20 , 标定板不能距离激光超过0.6m, 标定板上的激光点肯定多余 8 个
                {
                    // ROS_INFO("register new seg");
            ...
            nextPt = nextPt + skip;
            // ROS_INFO("1 pt: %d %d", currentPt, nextPt);
        } else
        {
            if(d1 > range_max)
            {
            ...
            // ROS_INFO("pt: %d %d", currentPt, nextPt);
            nextPt = nextPt + skip;
        }
    }

// ROS_INFO("pt: %d %d", segs.size(),  points.size());
    // 对 segs 的边界进行扩充
    for (int i = 0; i < segs.size(); ++i) {
        LineSeg tmp = segs.at(i);

        for (int j = 1; j < 4 &&  tmp.id_end + j < points.size(); ++j)
        {
        ...
        for (int j = -1; j > -4 && tmp.id_start + j >= 0; --j)
        {
