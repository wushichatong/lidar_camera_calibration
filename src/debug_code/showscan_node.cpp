...
//        std::cout << "---------- read scan -----------"<<std::endl;
    ...
    void showScanCallback(const sensor_msgs::LaserScanConstPtr &scan_in
                          ...
                          ,const sensor_msgs::ImageConstPtr &infra_image_in
                          ) {
...
//        cv::projectPoints(pts, cv::Vec3d::zeros(), cv::Vec3d::zeros(), cvK_, cvD_, pixel);
        ...
        for (size_t j = 0; j < pixel.size(); ++j) {
            cv::circle(img_src, pixel[j],3, cv::Scalar(255,0,0),1);
        }
        cv::imshow("show", img_src);
        writer<<img_src;
        infra_writer<<cv_bridge::toCvShare(infra_image_in, "bgr8")->image;
        cv::waitKey(10);
    }
...
int main(int argc, char **argv)
{
    ...
    std::string infra_img_topic_name;
    std::string bag_path;
    ...
    fsSettings["infra_img_topic_name"] >> infra_img_topic_name;
...
/// ==========================================
    ...
    message_filters::Subscriber<sensor_msgs::Image> infra_image_sub(nh, infra_img_topic_name, 10);

    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::LaserScan, sensor_msgs::Image, sensor_msgs::Image> sync_pol;
    message_filters::Synchronizer<sync_pol> sync(sync_pol(10), scan_sub, image_sub, infra_image_sub);
    sync.registerCallback(boost::bind(&sc::showScanCallback, _1, _2, _3));
